...
- For the most readable and comprehensible table of contents, your book's headings should ideally be *strictly hierarchical*: i.e. they shouldn't "skip levels" when /increasing/ in depth, for example jumping from level 2 directly to level 4 (the opposite, when /decreasing/ in depth, is of course acceptable). The ToC feature will still work if your headings don't fit this description, and a warning will be emitted at build time.
