...
- The second field should be either =filename=, =filepath=, or =contents= (or =f=, =p=, or =c= respectively), in lowercase. This is the /scope/ of the rule.
...
As is hopefully implied, the =filename= scope matches filenames, the =filepath= scope matches file paths (not including filenames), and the =contents= scope matches file contents. More interesting is the =mode= field. The =exclude= mode operates just like argument-type exclusions, and simply excludes any file which matches the rule from the build. The =include= scope inverts this functionality: /only/ files which match the rule will be /included/ in the build.
