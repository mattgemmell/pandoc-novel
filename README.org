...
Note that this project makes use of [[https://github.com/mattgemmell/FigureMark/][the FigureMark project]], configured as a git submodule. You shouldn't need to do anything special to make that work, but there's some useful [[https://git-scm.com/book/en/v2/Git-Tools-Submodules][information on git submodules]] here.
...
To provide a table of contents, simply insert the ={toc}= placeholder anywhere in your book's Markdown content, on a line of its own and without any leading whitespace. The placeholder will be replaced at build time with a hierarchical table of contents of all the Markdown headers in your book, using nested unordered lists in Markdown. The following nuances are noteworthy:
...
- Because the generated list is in Markdown format, your headings must be *strictly hierarchical*; in particular, they cannot "skip levels" when /increasing/ in depth, i.e. jump from level 2 directly to level 4 (the opposite, when /decreasing/ in depth, is of course acceptable).
