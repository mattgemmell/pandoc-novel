...
*** The same CSS stylesheets are used for every build; how can I customise the output for a given book?
...
- Add a =css= entry to your book's metadata JSON file, whose value is the filename of your custom CSS stylesheet. The stylesheet should reside in the same directory as the metadata file.
...
*** My Markdown content requires pre-processing of some kind before collation for publishing; is this possible?
