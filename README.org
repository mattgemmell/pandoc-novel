...
| =--process-toc= | Enable the processing of [[#tables-of-contents][tables of contents]]. Enabled by default. |
...
To provide a table of contents, simply insert the ={toc}= placeholder anywhere in your book's Markdown content, on a line of its own and without any leading whitespace. The placeholder will be replaced at build time with a hierarchical table of contents of all the Markdown headers in your book, using nested ordered lists (=OL= tags). The following nuances are noteworthy:

- The entire table of contents will be contained within a =DIV= with the CSS class =toc=.
- The generated list items (=LI= tags) will have CSS classes corresponding to the level of header they link to, for example =level-2=.
