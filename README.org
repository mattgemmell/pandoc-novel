...
You may wish to exclude certain Markdown files within the input folder structure from being included in your books, and you can readily do so via the exclusions feature. Exclusions are specified as regular expressions, and they can be specified in two different ways (which can be used separately or together).
...
exclude	filename	*		(?i)^_		Filename begins with underscore
