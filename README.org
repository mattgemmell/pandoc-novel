...
3. In the terminal, change to the directory containing your book's metata JSON file, and execute the =build-book.sh= script (inside the =novel-export= folder), passing it three parameters in the following order:
