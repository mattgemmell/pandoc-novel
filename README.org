...
If you need to exclude certain Markdown files based on your book's metadata, this is possible by using a proprietary flag in the regular expression pattern of an exclusion rule. The flag is =M= (in uppercase; not to be confused with =m= which means multi-line mode), and it indicates to the exclusions feature that you wish to have the pattern implicitly rewritten before being applied, replacing any metadata keys with their values.
